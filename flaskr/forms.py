...
def validate_legnth(form, field):
     ...
     if hours[::-1].find('.') > 2:
          raise ValidationError("Hours amounts cannot have more than two decimal places.")
...
def validate_date(form, field):
     ...
     if field.data > datetime.date.today() or field.data.year < datetime.date.today().year:
          raise ValidationError("Please enter a date that is within this year and not in the future.")


class AddHours(FlaskForm):
     description = TextField('Description', validators=[DataRequired(), Length(max=100, message="Please enter a description with less than 100 characters.")])
     hours = DecimalField('Hours', validators=[DataRequired(), validate_legnth, NumberRange(min=0, max=100, message='Please enter a number between 0 and 100')])
     group = SelectField("Group", validators=[DataRequired()])
     teacher = SelectField("Teacher", validators=[DataRequired()])
     date = DateField('Date Completed', format='%d/%m/%y', validators=[DataRequired(message="Please enter a valid date in the format dd/mm/yy."), validate_date])
